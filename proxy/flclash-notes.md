...
**位置:工具 → 备份与恢复**,页面分「远程」(WebDAV)和「本地」(文件)两区。

1. **两端各绑定一次** —— 「远程」区 → 绑定,填四项:

   | 字段 | 说明 |
   |---|---|
   | `uri` | WebDAV 服务器地址 |
   | `user` / `password` | 凭据 |
   | `fileName` | 默认 `backup.zip`,**两端必须一致** |

   绑好后先看**连通性**是否通。文件实际落在 `/FlClash/<fileName>`(`/FlClash` 目录 FlClash 自建)。

2. **桌面推**:「远程」区 → **备份**
3. **手机拉**:「远程」区 → **恢复** → ⚠️ 选「**仅恢复配置文件**」
4. **手机选中自己的 profile**(见下节双 profile 方案),选一次即可
5. 回配置页**重新激活订阅**,让脚本执行

**把桌面定为单一写入源。** 反方向别做——手机点「备份」会把手机状态推上去覆盖桌面的版本,
两边就打架了。日常是「桌面改 → 备份 → 手机恢复」单向流动。
...
**现象**:手机上规则模式下 Claude app 打不开、app 内报 error;切**全局模式 + 手动选节点**
就正常。而浏览器访问 `claude.ai` 在规则模式下是通的。

**排除项**(靠证据逐个排掉,别靠猜):

| 怀疑 | 证据 | 结论 |
|---|---|---|
| 节点坏 | 换测试 URL 后 `generate_204` 仅 61ms | ❌ 节点健康 |
| 进程匹配失效 | 连接页正确显示 `com.anthropic.claude` 且走了目标节点 | ❌ 工作正常 |
| 脚本没执行 | 浏览器访问 `claude.ai` 确实走了目标地区 | ❌ 脚本生效 |

> ⚠️ 测延迟别用业务域名当测试 URL。用 `https://claude.ai/ping` 测出 1.3–5.3 秒,
> 换成标准的 `generate_204` 只有 61ms —— 前者跑的是完整 TLS + 重定向,不是链路延迟。

**根因**:订阅的兜底是 `MATCH → <主组> → Auto → 🇭🇰 香港`,而 **Anthropic 屏蔽香港**
(同 OpenAI 的 `unsupported_country` 一个道理)。脚本只覆盖了 `anthropic.com` /
`claude.ai` / `claudeusercontent.com`,**漏了 `claude.com`**(Anthropic 近年在往该域名迁移)。
凡未覆盖的 claude 子域 → 落兜底 → 香港 → 被拒。浏览器访问的恰好是已覆盖的 `claude.ai`,所以通;
全局模式绕过所有规则,所以也通。

**修法**:补 `DOMAIN-SUFFIX,claude.com`,并加 `DOMAIN-KEYWORD,claude` 兜底——
漏掉任何 claude 子域都不会再落到被封锁的地区。

> 📌 **通用教训**:配「某服务走某地区」时,不能只列已知域名,**必须确认兜底链路的落地地区**
> 是否会踩该服务的地区封锁。逐级查选择链:
> `curl -s --noproxy '*' http://127.0.0.1:9090/proxies`
>
> 域名关键字兜底(`DOMAIN-KEYWORD`)比穷举 `DOMAIN-SUFFIX` 稳,也比 `PROCESS-NAME` 稳
> ——后者依赖「查找进程」开关(见下),而域名规则不依赖任何开关。

### ⚠️ 「查找进程」开关不随 WebDAV 同步

`find-process-mode`(界面叫「**查找进程**」,配置 → 通用)在 `patchClashConfig` 里,
而「仅恢复配置文件」**不碰该结构**。所以它是**每台设备独立**的。

关掉时,**全部 `PROCESS-NAME` 规则静默失效** —— 不报错、不提示,规则列表里照常显示,
但永远匹配不上。换机 / 重装后需手动确认开启。
